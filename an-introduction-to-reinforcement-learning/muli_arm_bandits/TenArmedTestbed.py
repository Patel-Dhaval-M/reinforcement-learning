...
#Version 2:                                                           #
# Name : Dhaval patel                                                 #
# Added moving target code and plots are moved to jupyter notebooks   #
# Plot 2.6 changes for python 3.5.X                                   #
#######################################################################
...
class Bandit:
    ...
    def __init__(self, kArm=10, epsilon=0., initial=0., stepSize=0.1, sampleAverages=False, UCBParam=None,
                 gradient=False, gradientBaseline=False, nonStationary=False, trueReward=0., shift=0.):
        self.k = kArm
...
#            else:
    ...
    def takeAction(self, action):
        ...
        if (self.nonStationary):
            self.qTrue[action] += (self.shift if np.random.rand()>0.5 else -self.shift)
